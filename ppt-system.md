...
/* Nav dots — EXACTLY ONE global container before the slides. NEVER place .dots inside a <section>.
   Dots are generated by JS from the actual slide count — keep the container empty (no hardcoded <div class="dot">). */
...
Die Punkte werden **dynamisch aus der Folienzahl erzeugt** — der `.dots`-Container bleibt leer, kein hartcodierter `<div class="dot">`-Block. `dotsWrap.innerHTML=''` ist ein Guard, der versehentlich reingepustete statische Punkte neutralisiert. So gibt es eine einzige Quelle der Wahrheit und nie Doppelbefüllung.
...
let cur = 0;
const lightBg = {'bg-l':1,'bg-w':1,'bg-y':1};

// Build dots dynamically from the actual slide count — single source of truth.
// Guard clears any stale/hardcoded <div class="dot"> so we never render duplicates.
dotsWrap.innerHTML = '';
slides.forEach((s,i) => {
...
function syncDots() {
  const act = slides[cur];
  let light = false;
  for (const k in lightBg) { if (act.classList.contains(k)) light = true; }
  dots.forEach((d,i) => {
...
<!-- Navigation dots (generated by JS from the slide count — keep this container empty) -->
<div class="dots"></div>
